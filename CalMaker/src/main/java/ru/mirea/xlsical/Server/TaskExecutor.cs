...
namespace ru.mirea.xlsical.Server
{
    ...
    /// Используйте <see cref="add(PackageToCalMaker)"/> для добавления задания.
    /// Используйте <see cref="take" для получения ответа.
    /// </summary>
    public class TaskExecutor
    {
        private readonly BlockingCollection<PackageToCalMaker> qIn;
        ...
        {
            this.qIn = new BlockingCollection<PackageToCalMaker>(new ConcurrentQueue<PackageToCalMaker>());
        ...
        public void add(PackageToCalMaker pack) => qIn.Add(pack);
        ...
        /// <summary>
        /// Берёт из входной очереди <see cref="add(PackageToCalMaker)"/> входной элемент,
        ...
        /// <returns>Пакет от обработчика.</returns>
        public PackageToProviderHTTP monoStep(PackageToCalMaker pkg)
        {
            if (pkg == null)
            {
